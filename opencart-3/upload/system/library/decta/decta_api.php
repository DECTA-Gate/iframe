...
define('ROOT_URL', 'https://gate.dcstage.dvte.ch');
...
class DectaAPI
{
    ...
    public function call($method, $route, $params = array())
    {
        ...
        $this->log_info('Authorization:');
        $this->log_info(var_export($authorization_header, true));
        $response = curl_exec($ch);

        if (!$response) {
            $this->log_error('cURL', curl_error($ch));
            $info = curl_getinfo($ch);
            $this->log_info('Curl info:');
            $this->log_info(var_export($info, true));
        }
